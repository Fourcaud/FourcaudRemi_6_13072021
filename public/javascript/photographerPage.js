...
let appMedia = new (function () {
  ...
  this.FetchAll = function (data) {
    // Selection de l'élément
    const PhotographersID = 925;
    let elPhotographers = document.getElementById("media");

    let htmlMedia = "";

    let resultat = data.find((data) => data.photographerId === PhotographersID);
    ...
    for (let y in resultat) {
      ...
      htmlMedia +=
        '<div class="card__name"><h2>' + resultat.title + "</h2></div>";

      htmlMedia +=
        '<div class="card__tagline"><h3>' + resultat.likes + "</h3></div>";
      htmlMedia += '<div class="card__tags ">';
      for (let j in resultat.tags) {
        htmlMedia += '<h6 class="labeltags">#' + resultat.tags[j] + "</h6>";
      }
      htmlMedia += "</div>";

      htmlMedia += "</article>";
      elPhotographers.innerHTML = htmlMedia;
    }
  };
})();
